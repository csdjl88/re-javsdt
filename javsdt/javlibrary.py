...
# Cookie = input("请输入javlibrary的Cookie")
Cookie = "timezone=-480; __utma=45030847.1445723357.1758730787.1758730787.1758730787.1; __utmz=45030847.1758730787.1.1.utmcsr=(direct)|utmccn=(direct)|utmcmd=(none); __utma=45030847.1445723357.1758730787.1758730787.1758730787.1; __utmz=45030847.1758730787.1.1.utmcsr=(direct)|utmccn=(direct)|utmcmd=(none); dm=javlibrary; over18=18; cf_clearance=Rq91syYONbx59HvCxcUrrWWw.x5a4vQX3Zb79VyrpNM-1772213869-1.2.1.1-Z2NBkuqgvSCAIr_x8MW26Bz22mBNp8Orb3uEPUOJ0qYb0UIc.1oGB_h5.MuSwg_ffEwdvPdvWwT0XDwZgzTvyS0NQjvDeMrvVeCMbI3SwY.SbcayvmAI5a6aodGV3Q7s8a4JlSkocGPhiBle41lKxm7ccL.rnO4w7kJTosBG_RvqgXx4Jzmfmy097GCf0qx9aQKxq1gHlGH04tR1AuCjDwriacc7XtbC7.pD8IoQYlBf7wN6rkkw8tmMISUINu28"
try:
    config_settings = RawConfigParser()
    config_settings.read('【点我设置整理规则】.ini', encoding='utf-8-sig')
    ##########################################################################
    # 是否 收集nfo
    bool_nfo = True if config_settings.get(
        "收集nfo", "是否收集nfo？") == '是' else False
    ...
    bool_strip_actors = True if config_settings.get(
        "收集nfo", "是否去除标题末尾可能存在的演员姓名？") == '是' else False
    ...
    # 是否 将 片商 作为特征
    bool_tag = True if config_settings.get(
        "收集nfo", "是否将特征保存到tag？") == '是' else False
    ##########################################################################
    # 是否 重命名 视频
    bool_rename_mp4 = True if config_settings.get(
        "重命名影片", "是否重命名影片？") == '是' else False
    # 自定义 重命名 视频
    custom_video = config_settings.get("重命名影片", "重命名影片的格式")
    # 是否 重命名视频所在文件夹，或者为它创建独立文件夹
    bool_rename_folder = True if config_settings.get(
        "修改文件夹", "是否重命名或创建独立文件夹？") == '是' else False
    # 自定义 新的文件夹名
    custom_folder = config_settings.get("修改文件夹", "新文件夹的格式")
    ##########################################################################
    # 是否 重命名用户已拥有的字幕
    bool_rename_subt = True if config_settings.get(
        "字幕文件", "是否重命名已有的字幕文件？") == '是' else False
    ##########################################################################
    # 是否 归类jav
    bool_classify = True if config_settings.get(
        "归类影片", "是否归类影片？") == '是' else False
    # 是否 针对“文件夹”归类jav，“否”即针对“文件”
    bool_classify_folder = True if config_settings.get(
        "归类影片", "针对文件还是文件夹？") == '文件夹' else False
    # 自定义 路径 归类的jav放到哪
    ...
    ##########################################################################
    # 是否 下载图片
    bool_jpg = True if config_settings.get(
        "下载封面", "是否下载封面海报？") == '是' else False
    # 自定义 命名 大封面fanart
    ...
    bool_watermark_subt = True if config_settings.get(
        "下载封面", "是否为海报加上中文字幕条幅？") == '是' else False
    # 是否 如果视频是“无码流出”，给poster的右上角加上“无码流出”的斜杠
    bool_watermark_divulge = True if config_settings.get(
        "下载封面", "是否为海报加上无码流出条幅？") == '是' else False
    ##########################################################################
    # 是否 收集演员头像
    bool_sculpture = True if config_settings.get(
        "kodi专用", "是否收集演员头像？") == '是' else False
    # 是否 对于多cd的影片，kodi只需要一份图片和nfo
    bool_cd_only = True if config_settings.get(
        "kodi专用", "是否对多cd只收集一份图片和nfo？") == '是' else False
    ##########################################################################
    # 是否 使用局部代理
    bool_proxy = True if config_settings.get(
        "局部代理", "是否使用局部代理？") == '是' else False
    # 是否 使用http代理，否 就是socks5
    bool_http = True if config_settings.get(
        "局部代理", "http还是socks5？") == 'http' else False
    # 代理端口
    custom_proxy = config_settings.get("局部代理", "代理端口")
    # 是否 代理javlibrary
    bool_web_proxy = True if config_settings.get(
        "局部代理", "是否代理javlibrary？") == '是' else False
    # 是否 代理javbus，还有代理javbus上的图片cdnbus
    bool_bus_proxy = True if config_settings.get(
        "局部代理", "是否代理javbus？") == '是' else False
    # 是否 代理arzon
    bool_arzon_proxy = True if config_settings.get(
        "局部代理", "是否代理arzon？") == '是' else False
    # 是否 代理dmm图片，javlibrary和javdb上的有码图片几乎都是直接引用dmm
    bool_dmm_proxy = True if config_settings.get(
        "局部代理", "是否代理dmm图片？") == '是' else False
    ##########################################################################
    ...
    bool_review = True if config_settings.get(
        "信息来源", "是否用javlibrary整理影片时收集网友的热评？") == '是' else False
    # 是否 收集javlibrary下方用户超过10个人点赞的评论
    bool_bus_first = True if config_settings.get(
        "信息来源", "是否用javlibrary整理影片时优先从javbus下载图片？") == '是' else False
    # 自定义 无视的字母数字 去除影响搜索结果的字母数字 xhd1080、mm616、FHD-1080
    custom_surplus_words = config_settings.get("原影片文件的性质",
                                               "无视有码、素人视频文件名中多余的形如abc123的字母数字")
    # 是否 需要简介
    bool_plot = True if config_settings.get(
        "百度翻译API", "是否需要日语简介？") == '是' else False
    # 是否 把日语简介翻译为中文
    bool_tran = True if config_settings.get(
        "百度翻译API", "是否翻译为中文？") == '是' else False
    # 账户 百度翻译api
...
header = steal_library_header(
    url_web,
    proxy_lib,
    Cookie)  # 获得能通过javlibrary检测5秒的请求头部
# 初始化：哪些站点需要代理
...
dict_nfo = {
    ...
    '车牌前缀': 'ABC',
    '是否流出': '',
    '影片类型': custom_movie_type,
    '系列': '有码系列',
    '原文件名': 'ABC-123',
    '原文件夹名': 'ABC-123',
}
# 初始化：将ini读取的各种自定义string，切割为list。如果是dict_nfo没有的、用户自己需要的元素，将它们放进dict_nfo中。
list_extra_genres = custom_genres.split(
    '、') if custom_genres else []  # 需要的额外特征
...
list_name_nfo_title = custom_nfo_title.replace('标题', '完整标题', 1).split(
    '+')  # nfo中title的写法。给用户命名用的标题可能是删减的，nfo中的标题是完整标题
...
for i in custom_classify_basis.split(
        '\\'):  # 归类标准，归类到哪个文件夹。不管是什么操作系统，用户写归类的规则，暂时用“\”连接
    for j in i.split('+'):
        if j not in dict_nfo:
            dict_nfo[j] = j
...
while input_start_key == '':
    ...
    for root, dirs, files in os.walk(root_choose):
        ...
        # 存放：这一层文件夹下的几个车牌 abp avop snis。。。{'abp': 1, avop': 2}
        # abp只有一集，avop有cd1、cd2
        dict_car_pref = {}
        ...
        for file_raw in files:
            file_temp = file_raw.upper()
            if file_temp.endswith(
                    tuple_type) and not file_temp.startswith('.'):
                ...
                if jav_num:
                    ...
                    # 当前jav，是第几集  {'abp': 1, avop': 2}
                    jav_struct.episodes = dict_car_pref[jav_num]
                    # 这个车牌在dict_subt_files中，有它的字幕。
                    if jav_num in dict_subt_files.values():
                        jav_struct.subt = list(dict_subt_files.keys())[
                            list(dict_subt_files.values()).index(jav_num)]
        ...
        for jav in list_jav_videos:
            ...
            # 影片的相对于所选文件夹的路径，用于报错
            path_relative = sep + path_jav.replace(root_choose, '')
            print('>>正在处理：', jav_file)
            ...
            if subt_file:
                ...
                bool_subt = check_subt_divulge(
                    root, jav_name, list_subt_video, '中文字幕')
                dict_nfo['是否中字'] = custom_subt_expression if bool_subt else ''
            # 判断是否是无码流出的作品
            bool_divulge = check_subt_divulge(
                root, jav_name, list_divulge_video, '无码流出')
            ...
            try:
                ...
                if '图书馆' in jav_file:  # 用户指定了网址，则直接得到jav所在网址
                    ...
                    if str(url_appointg) != 'None':
                        url_search_web = url_web + \
                            '?v=' + url_appointg.group(1)
                    else:
                        num_fail += 1
                        record_fail(
                            '    >第' +
                            str(num_fail) +
                            '个失败！你指定的library网址有错误：' +
                            path_relative +
                            '\n')
                        continue  # 【退出对该jav的整理】
                # 用户没有指定网址，则去搜索
                ...
                html_web, header = get_library_html(
                    url_search_web, header, proxy_lib)
                # 搜索结果的网页，大部分情况就是这个影片的网页（因为javlibrary会自动跳转），也有可能是多个结果的网页
                # 尝试找标题，第一种情况：找得到，就是这个影片的网页
                titleg = search(
                    r'<title>([A-Z].+?) - JAVLibrary</title>',
                    html_web)  # 匹配处理“标题”
                # 搜索结果就是AV的页面
                if str(titleg) != 'None':
                ...
                # 第二种情况：搜索结果可能是两个以上，所以这种匹配找不到标题，None！
                    ...
                    list_search_results = findall(
                        r'v=javli(.+?)" title="(.+?-\d+?[a-z]? .+?)"',
                        html_web)  # 这个正则表达式可以忽略avop-00127bod，它是近几年重置的，信息冗余
                    # 从这些搜索结果中，找到最正确的一个
                    if list_search_results:
                        # 默认用第一个搜索结果
                        url_first_result = url_web + \
                            '?v=javli' + list_search_results[0][0]
                        # 在javlibrary上搜索 SSNI-589 SNIS-459 这两个车牌，你就能看懂下面的if
                        if len(list_search_results) > 1 and not list_search_results[1][1].endswith(
                                'ク）'):  # ク）是蓝光重置版
                            # print(list_search_results)
                            if list_search_results[0][1].endswith(
                                    'ク）'):  # 排在第一个的是蓝光重置版，比如SSNI-589（ブルーレイディスク），它的封面不正常，跳过它
                                url_first_result = url_web + \
                                    '?v=javli' + list_search_results[1][0]
                            elif list_search_results[1][1].split(' ', 1)[
                                    0] == jav_raw_num:  # 不同的片，但车牌完全相同，比如id-020。警告用户，但默认用第一个结果。
                                bool_unique = False
                                num_fail += 1
                                record_fail(
                                    '    >第' +
                                    str(num_fail) +
                                    '个警告！javlibrary搜索到同车牌的不同视频：' +
                                    jav_raw_num +
                                    '，' +
                                    path_relative +
                                    '\n')
                        ...
                        html_web, header = get_library_html(
                            url_first_result, header, proxy_lib)
                        # 找到标题，留着马上整理信息用
                        title = search(
                            r'<title>([A-Z].+?) - JAVLibrary</title>',
                            html_web).group(1)
                ...
                if 'T-28' in jav_temp:
                    ...
                    if jav_tempg:
                        jav_temp = jav_tempg.group(
                            1) + 'ID-' + jav_tempg.group(2)
                ...
                # javlibrary的精彩影评   (.+?\s*.*?\s*.*?\s*.*?)
                # 下面的匹配可能很奇怪，没办法，就这么奇怪
                if bool_review:
                    ...
                    if list_all_reviews:
                        ...
                        for rev in list_all_reviews:
                            list_reviews = findall(
                                r'hidden">([\s\S]*?)</textarea>', rev, re.DOTALL)
                            if list_reviews:
                ...
                # 有大部分信息的html_web
                html_web = search(
                    r'video_title"([\s\S]*?)favorite_edit',
                    html_web,
                    re.DOTALL).group(1)
                # DVD封面cover
                coverg = search(
                ...
                if str(scoreg) != 'None':
                    ...
                    # javlibrary上稍微有人关注的影片评分都是6分以上（10分制），强行把它差距拉大
                    float_score = (float_score - 4) * 5 / 3
                    if float_score >= 0:
                ...
                # path_jav 是现在视频路径   path_subt是现在字幕路径   subt_file是 自始至终的
                # 字幕文件名（带文件类型）  jav_folder是自始至终的 所在文件夹名（不是路径）
                # dict_nfo['视频']， 先定义为原文件名，【即将变化】
                dict_nfo['视频'] = dict_nfo['原文件名'] = jav_name
                jav_folder = dict_nfo['原文件夹名'] = root.split(
                    sep)[-1]  # 当前影片的文件夹名称，【即将变化】
                ...
                if bool_rename_mp4:
                    ...
                    for j in list_rename_video:
                        jav_name += dict_nfo[j]
                    # 【发生变化】jav_name  去除末尾空格，否则windows会自动删除空格，导致程序仍以为带空格
                    jav_name = jav_name.rstrip() + str_cd
                    # 新的完整文件名，新的路径
                    ...
                    # 已存在目标文件，但就是现在的文件
                        ...
                        try:
                            os.rename(path_jav, path_jav_new)
                            path_jav = path_jav_new  # 【发生变化】 path_jav
                        except BaseException:  # windows本地磁盘，“abc-123.mp4”重命名为“abc-123.mp4”或“ABC-123.mp4”没问题，但有用户反映，挂载的磁盘会报错“file exists error”
                            num_fail += 1
                            record_fail(
                                '    >第' +
                                str(num_fail) +
                    ...
                    # 存在目标文件，不是现在的文件。
                        ...
                        record_fail(
                            '    >第' +
                            str(num_fail) +
                            '个失败！重命名影片失败，重复的影片，已经有相同文件名的视频了：' +
                            path_jav_new +
                            '\n')
                        continue  # 【退出对该jav的整理】
                ...
                # 2
                # 归类影片【和其他整理模式没区别】只针对视频文件和字幕文件。注意：第2操作和（下面第3操作或者下面第7操作）互斥，只能执行一个，归类影片是针对“文件”还是“文件夹”。
                if bool_classify and not bool_classify_folder:
                    ...
                    for j in list_classify_basis:
                        ...
                        )  # 【临时变量】归类的目标文件夹路径    C:\Users\JuneRain\Desktop\测试文件夹\1\葵司\
                    ...
                    if not exists(path_jav_new):
                        ...
                        record_fail(
                            '    >第' +
                            str(num_fail) +
                            '个失败！归类失败，重复的影片，归类的目标文件夹已经存在相同的影片：' +
                            path_jav_new +
                            '\n')
                        continue  # 【退出对该jav的整理】
                ...
                if bool_rename_folder:
                    ...
                    if bool_separate_folder:
                        if jav_epi == num_all_episodes:
                            ...
                            del list_root_now[-1]
                            # 【临时变量】新的影片路径。上级文件夹路径+新文件夹名称=新文件夹路径
                            root_new = sep.join(
                                list_root_now) + sep + jav_folder_new
                            # 想要重命名的目标影片文件夹不存在
                    ...
                    # 不是独立的文件夹，建立独立的文件夹
                        ...
                        # 【临时变量】新的影片所在文件夹。 当前文件夹的上级文件夹路径+新文件夹=新独立的文件夹的路径
                        root_separate_folder = root_now + sep + jav_folder_new
                        # 确认没有同名文件夹
                        ...
                        if not exists(path_jav_new):
                            ...
                            if subt_file:
                                os.rename(
                                    path_subt, root_separate_folder + sep + subt_file)
                ...
                if bool_jpg:
                    ...
                    # emby需要多份，现在不是第一集，直接复制第一集的图片
                        ...
                        try:
                            copyfile(
                                path_fanart.replace(
                                    str_cd, '-cd1'), path_fanart)
                            print('    >fanart.jpg复制成功')
                            copyfile(
                                path_poster.replace(
                                    str_cd, '-cd1'), path_poster)
                    ...
                    if check_pic(path_fanart):
                        ...
                        if bool_unique and bool_bus_first and url_cover_bus and '图书馆' not in jav.file:
                            ...
                            try:
                                download_pic(
                                    url_cover_bus, path_fanart, proxy_bus)
                                ...
                                try:
                                    download_pic(
                                        url_cover, path_fanart, proxy_dmm)
                                    print('    >fanart.jpg下载成功')
                                except BaseException:
                                    num_fail += 1
                                    record_fail(
                ...
                if bool_classify and bool_classify_folder and jav_epi == num_all_episodes:  # 需要移动文件夹，且，是该影片的最后一集
                    ...
                    root_now_new = root_dest + \
                        jav_folder  # 【临时变量】 文件夹移动的目标路径   C:\Users\JuneRain\Desktop\测试文件夹\1\葵司\【葵司】AVOP-127\
                    # print(root_now_new)
                    # 归类的目标路径 还没有这个车牌的文件夹
                    if not exists(root_now_new):
                        ...
                        for i in jav_files:
                            os.rename(
                                root_now + sep + i, root_now_new + sep + i)
